...
@inject('orders', 'App\Services\Orders')

@section('content')
    <div class="page-heading">
        <h1><i class='fa fa-edit'></i> Pedidos</h1>
                        ...
                        <table data-sortable class="table table-hover table-striped">
                            ...
                            @foreach($orders->getAll() as $order)
                                <tr>
                                    ...
                                    <td>{{ $order->fecha }}</td>
                                    <td>{{ $order->customer->cedula }}</td>
                                    <td>{{ $order->ivaType->tipo }}</td>
                                    <td>{{ $order->total }}</td>
                                    <td>
                                        <div class="btn-group btn-group-xs">
                                            <a href="{{route('admin.orders.show', $order->codigo)}}" data-toggle="tooltip" title="Ver más" class="btn btn-info"><i class="fa fa-info-circle"></i></a>
